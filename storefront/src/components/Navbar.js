            ...
            <nav className="navbar">
                <div className="categories">
                    {this.props.categories.map(({name}) => {
                        return(name === this.props.defaultCategory.name ? <button key={name} className="category active"><Link to="/">{name}</Link></button> : <button className="category" key={name} onClick={this.setDefaultCategory({name})}><Link to="/">{name}</Link></button>)
                    })}
                </div>
                ...
                <div className="dropdowns">
                    <div className="currency-div">
                        <button onClick={this.openCurrencyDropdown} className="currency-icon">{this.props.defaultCurrency.symbol}<span className="currency-drop-icon">&#8743;</span></button>
                        <ul className="currency-dropdown" hidden>
                            {this.props.currencies.map(({symbol, label}) => <li key={symbol}><button value={symbol} onClick={this.setDefaultCurrency}>{symbol} {label}</button></li>)}
                        </ul>
                    </div>
                    <div className="cart-div">
                        <button className="cart-img" onClick={this.openCart}><img src={process.env.PUBLIC_URL+'/cart.png'} alt='cart-img'></img></button>
                        <button className="cart-count">{calculateQuantity(this.props.cart)}</button>
                        <CartOverlay/>
                    </div>
                </div>
            </nav>
