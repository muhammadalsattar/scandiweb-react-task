...
export class Product extends React.Component {
    async componentDidMount() {
        ...
        document
            .querySelectorAll('.product-attribute-values button')
            .forEach(button => {
        ...
        const html = htmlParser.parseFromString(
    ...
    changeActiveAttr = e => {
        e.preventDefault();
        document
            .querySelector(`.${e.target.parentNode.className}#selected`)
            .removeAttribute('id');
    ...
    addToCart = product => {
        let selectedAttr = [];
        document
            .querySelectorAll('.product-attribute-values div#selected')
            .forEach(node => {
                selectedAttr.push({
                    name: node.className,
                        ...
                        <div className="product-gallery-images">
                            {this.props.product?.gallery.map(image => (
                                <img
                                    key={image}
                                    src={image}
                                    onClick={this.changeFocuesdImage}
